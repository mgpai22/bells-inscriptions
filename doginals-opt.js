...
async function wallet() {
    ...
    if (subcmd === 'new') {
        ...
        await walletSync(process.argv[4])
...
function walletNew() {
    ...
    for (let i = 1; i <= 69; i++) {
        const walletPath = `wallet_${i}.json`;
        ...
        if (!fs.existsSync(walletPath)) {
            ...
            fs.writeFileSync(walletPath, JSON.stringify(json, 0, 2))
            console.log('address', address)
        } else {
            console.log(`Wallet at ${walletPath} already exists`);
        }
...
async function walletSync(walletNum) {
    if (process.env.TESTNET === 'true') throw new Error('no testnet api')
    const walletPath = `wallet_${walletNum}.json`;

    let wallet = JSON.parse(fs.readFileSync(walletPath))
    ...
    fs.writeFileSync(walletPath, JSON.stringify(wallet, 0, 2))
...
async function mint() {
    ...
    if (fs.existsSync(argContentTypeOrFilename)) {
        ...
        throw new Error('not supported fvck')
    ...
    console.log('minting', contentType, data.length, 'bytes', ' wallet path', walletPath)
